        ...
        if self.has_header:
            ...
            while read_keys == False:
                if lines[nheader][0] == '#':
                    ...
                    for k in keys:
                        self.keys[k] = keys.index(k)
                    # print keys
                    nheader += 1
                    if ('id' in self.keys) or ('type' in self.keys) or (
                            'radius' in self.keys) or ('x' in self.keys):
                        read_keys = True
                elif lines[0][0:5] == 'keys:':
                    ...
                    print(
                        "Error: There are no keys for these file, just comments! Cannot determine type of input!")
                    # print "Found " + str(nheader) + " lines of comments and
                    # keys "
                lines = lines[nheader:]
