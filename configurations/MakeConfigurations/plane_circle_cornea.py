...
class Plane:
    ...
    def write(self, outfile):
        ...
        out.write(
            '# id  type radius  x   y   z   vx   vy   vz   nx   ny   nz  omega\n')
        for p in self.particles:
            x, y, z = p.r
            vx, vy, vz = p.v
            nx, ny, nz = p.n
...
class Circle:
    def __init__(self, R, N, v, poly, boundary):
        ...
        # Dense boundary wall, linear packing fraction 1.5
        self.Nbound = int(1.5 * 2.0 * pi * R / 2.0)
        self.particles = [Particle(i, 1) for i in range(N)]
        self.boundparticles = [
            Particle(
                i, 2) for i in range(
                N, N + self.Nbound)]
        self.boundparticles.extend(
            [Particle(i, 3) for i in range(N + self.Nbound, N + 2 * self.Nbound)])
    ...
    def __generate_posinside(self):
        for i in range(self.N):
            ...
            while rpart > (self.R - 1.0)**2:
                ...
                rpart = x**2 + y**2
            self.particles[i].r = [x, y, z]
            self.particles[i].R = uniform(
                1 - 0.5 * self.poly, 1 + 0.5 * self.poly)
    ...
    def __generate_posbound(self):
        for i in range(self.Nbound):
            ...
            self.boundparticles[i].r = [
                self.R * cos(sval), self.R * sin(sval), 0]
            # introduce a little noise there; no ratchets here
            self.boundparticles[i].R = uniform(
                1 - 0.5 * self.poly, 1 + 0.5 * self.poly)
    ...
    def __generate_posbound2(self):
        for i in range(self.Nbound, 2 * self.Nbound):
            ...
            self.boundparticles[i].r = [
                (self.R + 0.5) * cos(sval), (self.R + 0.5) * sin(sval), 0]
            # introduce a little noise there; no ratchets here
            self.boundparticles[i].R = uniform(
                1 - 0.5 * self.poly, 1 + 0.5 * self.poly)
    ...
    def __generate_director(self):
        for p in self.particles:
            phi = uniform(0, 2 * pi)
            p.n = [cos(phi), sin(phi), 0.0]
        # This is actually intriguing: make them point inwards, outwards or
        # random
        for i in range(self.Nbound):
            if self.boundary == 2:
...
parser.add_argument("-b", "--boundary", type=int, default=0,
                    help="boundary n: 0 random 1 outward 2 inward")
...
# N = int(round(args.R**2*args.phi/4.0)) # An individual particle has area
# pi R^2 = 4 pi
